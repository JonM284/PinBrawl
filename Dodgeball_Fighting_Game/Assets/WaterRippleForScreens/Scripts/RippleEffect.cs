...
public class RippleEffect : MonoBehaviour {
    ...
    protected Camera m_cameraRef => CommonUtils.GetRequiredComponent(ref m_camera, () =>
    {
    ...
    {
        JuiceGameController.RequestRipple += SetNewRipplePosition;
    }
    ...
    public void SetNewRipplePosition(Vector3 _targetPos) {
        //Set new ID
        lastIDUsed = (lastIDUsed + 1 >= waveCount) ? 0 : lastIDUsed + 1;

        Vector2 _screenPoint = m_cameraRef.WorldToScreenPoint(_targetPos);
        
        //Convert to (0, 1) Space & set TargetPosition
        targetPosition[lastIDUsed] = ConvertToTextureSpace(_screenPoint);
